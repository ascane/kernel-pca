...
class Kmeans:
    ...
    def fit(self, data, niter=10, warm_start=False):
        ...
        if not(warm_start):
            ...
            index = numpy.random.randint(0, data.shape[0], (self.nclusters))
            self.mu = numpy.zeros((self.nclusters, self.dim))
            for i, idx in enumerate(index):
                self.mu[i] = data[idx, :]

        for _ in tqdm(range(niter)):
            self._assign_cluster(data)
            self._update_center(data)
